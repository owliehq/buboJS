 ...
 * @param options
 */
function prepareHttpError(statusCode: number, defaultMessage: string, options: OptionsErrorFactory | string) {
  if (typeof options === 'string') return new HttpError(statusCode, options)
  if (!options) return new HttpError(statusCode, defaultMessage)
